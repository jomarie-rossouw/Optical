...
kb = c.k/c.e
data = np.loadtxt('data/S2_centroids_Fitted.txt', delimiter = ',', skiprows=1)
T = data[:,0]
eV = data[:,1]
...
plt.savefig('fit_results/energy_s2/fermi_reg.png')
plt.show()
...
if save_or_nah in ('y', 'yes', 'slay'):
    ...
    org.save_fit(f'fit_results/{out_dir}/model1', x=T1, y=eV1, x_name = 'Temperature (K)', y_name = 'Peak Energy (eV)', res = res1, fitted_eq = fitted_eq1, param_values=param_values1)
    org.save_fit(f'fit_results/{out_dir}/model2', x=T2, y=eV2, x_name = 'Temperature (K)', y_name = 'Peak Energy (eV)', res = res2, fitted_eq = fitted_eq2, param_values=param_values2)
    org.save_fit(f'fit_results/{out_dir}/model3', x=T3, y=eV3, x_name = 'Temperature (K)', y_name = 'Peak Energy (eV)', res = res3, fitted_eq = fitted_eq3, param_values=param_values3)
