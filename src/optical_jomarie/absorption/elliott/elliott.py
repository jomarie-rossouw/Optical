...
def dirac_broadening(Ry, Eg, eV, a, alpha):
...
# constant n, a, en b
def elliott(a, b, n, alpha, Eg, Ry, eV):
    Exo = 0
    for n in range(n):
        n = n+1 #begin by n=1 ipv n=0 want anders doen mens (-1)! which DNE
        E_n = En(Eg, Ry, n, alpha)
        Exo += Exo_spectra(Ry,n,alpha)*dirac_broadening(Ry, Eg, eV, a, alpha)

    O_0 = O0(alpha, eV, Ry)
    absorption = O_0*(Exo + cont_spectra(alpha, gamma(Ry, eV))*heavi_broadening(eV, Eg, b))
    Eb = -Ry/(2+(alpha-3)/2)**2
    return absorption, Exo, Eb, E_n

if __name__ == '__main__':
    eV = np.linspace(1.5, 3.5, 1000)
    alpha, a, b, Eg, Ry, n = 2.75, 0.058, 1.79, 2.51, 0.089, 1
    absorp, Exo, Eb, E_n = elliott(a, b, n, alpha, Eg, Ry, eV)
