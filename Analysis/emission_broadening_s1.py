...
params1.add('Gamma_0', value=0.005) #skuif dit op en af 5.9e-3 is goed
params1.add('LA', value=0.001) #23 is goed - skuif dit op en af
params1.add('LO', value=0.137,)
params1.add('kb', value=kb, vary=False)
params1.add('E_LO', value=0.008) #impacts the curve, 0.0x waardes is n reguit lyn, by 10 begin dit curve, 20-30 gee n decent fitting van 83-203 K, na 30 raak dit weer reguit
...
plt.scatter(T, fwhm, color = '#ff7f0e')
plt.plot(T1, res1.best_fit, label = 'Model 1')
# plt.plot(T2, res2.best_fit, label = 'Model 2')
plt.xlabel('Temperature (K)')
plt.ylabel('FWHM (eV)')
plt.savefig('figures/S2_fwhm_fit_v1.png')
plt.show()

out_dir = 'fwhm_s2'
org.save_fit(f'fit_results/{out_dir}/fwhm2_v1', x=T1, y=fwhm1, y_new=res1.best_fit, x_name = 'Temperature (K)', y_name = 'FWHM (eV)', res = res1, fitted_eq = fitted_eq1, model_list = model_list1, param_values=params1)